...
const renderSkuColum = (size: number, i: number) => (
  value: string | SkuProp[],
  row: ProdSku,
  index: number,
) => {
  let val = value;
...
class SkuForm extends PureComponent<SkuFormProps, SkuFormState> {
  ...
  addProp = () => {
    ...
    const score = skuPropNames.length ? (last(skuPropNames) as SkuPropName).score + 10 : 10;
    skuPropNames.push({
      propName: '',
      propVals: [],
      key: `PROP_${score}`,
  ...
  filterSkuPropNames = (skuPropNames: SkuPropName[]): SkuPropName[] => {
    return filter(
      skuPropNames,
      ele => ele.propName && this.filterSkuPropVals(ele.propVals).length,
    ) as SkuPropName[];
  };
  ...
  reduceSku = () => {
    ...
    const newSkus = sortBy(
      reduce(
        skuProps,
        (result: ProdSku[], value: SkuProp[]) => {
          const ret: ProdSku[] = [];
          each(result, res => {
            each(value, next => {
              ret.push({
  ...
  render() {
    ...
    return (
      <Fragment>
        <Card
          actions={[
            <Button disabled={skuPropNames.length === this.deep} onClick={this.addProp}>
              添加规格项目
            </Button>,
          ]}
        >
          {skuPropNames.map((skuProp, skuPropIndex) => (
            <div key={skuProp.key} className={styles.skuPropWarp}>
              ...
              >
                ...
                <Icon
                  onClick={() => this.delProp(skuPropIndex)}
                  className={styles.skuPropDel}
                  type="close-circle"
                  theme="filled"
                />
              </Form.Item>
              <Form.Item {...formSubItemLayout} label="规格值">
                <div>
                  {skuProp.propVals &&
                    skuProp.propVals.map((propValue, propValueIndex) => (
                      <span key={propValue.key} className={styles.skuPropItem}>
                        <div className={styles.skuPropWrap}>
                          <Input
                            ...
                            onChange={e =>
                              this.handleSkuPropValChange(e, skuPropIndex, propValueIndex)
                            }
                            onKeyPress={e =>
                              this.handleSkuPropValPress(e, skuPropIndex, propValueIndex)
                            }
                          />
                          <Icon
                            onClick={() => this.delPropValue(skuPropIndex, propValueIndex)}
                            className={styles.skuPropValDel}
                            type="close-circle"
                            theme="filled"
                          />
                        </div>
                  ...
                  <a href="javascript:;" onClick={() => this.addPropVal(skuPropIndex)}>
                    添加规格值
                  </a>
                </div>
              </Form.Item>
            ...
            <ProdSkuTable dataSource={skus} pagination={false} bordered>
              {skuColums.map((column, i) => (
                <ProdSkuColumn
                  key={'propList[' + i + '].key'}
                  title={column.title}
                  dataIndex={'propList[' + i + '].propVal'}
                  render={renderSkuColum(column.rowSpan, i)}
                ></ProdSkuColumn>
              ))}
              <ProdSkuColumn
                title="价格"
                dataIndex="price"
                render={this.renderInput('price')}
              ></ProdSkuColumn>
              <ProdSkuColumn
                title="库存"
                dataIndex="quantity"
                render={this.renderInput('quantity')}
              ></ProdSkuColumn>
              <ProdSkuColumn
                title="规格编码"
                dataIndex="itemNo"
                render={this.renderInput('itemNo')}
              ></ProdSkuColumn>
            </ProdSkuTable>
          </Form.Item>
        )}
